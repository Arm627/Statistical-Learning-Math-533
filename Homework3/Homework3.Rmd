...
(c) Use the lm() function to perform a multiple linear regression with mpg as the response and all other variables except name as the predictors. Use the summary() function to print the results.
    ...
    Both F-statistic and p-value's value provide evidence against the null hypothesis. As a result, we can conclude there is a relationship between the predictors and the response. 
    Based on the p-value, displacement, weight, year, and origin have a statistically significant relationship to the response.
    The estimate coefficient for year is 0.750773, which indicates the mpg goes up 0.750773 per year. 


(d) Use the plot() function to produce diagnostic plots of the linear regression fit. 

    By the residual plot, we can conclude the model is not fit well since there is a non-linear pattern showing. Also, scale-location plot shows there is no point located outside of the range [-2,2], which indicates there may be no outliers. Point 14 is the high leverage point. 
    ```{r, fig.show="hold", out.width="50%"}
    plot(mlr)
    ```
...
(f) Try a few different transformations of the variables.
    ...
    The log(cylinders) transformation is more significant than cylinders, but the sqrt(acceleration) is less significant than acceleration. We assume that is dependent on the variables selection. 
    ```{r}
...
(a) Here, we fit a multiple regression model to predict Sales using Price, Urban, and US.
    ...
    summary(lm.fit)
    ```
(b) Our model's coefficients can be interpreted as follows.  As the price the company charges for car seats at each site increases by $1, our model predicts unit sales at each location decrease by 54 units.  If the store is in an urban location, our model predicts unit sales at each location decrease by 22 units.  If the store is in the United States, our model predicts unit sales at each location increase by 1,201.

(c) The model's equation form is given by
    $$
    Sales = 13.043469 -0.054459x_1 -0.021916x_2 +1.200573x_3.
    $$
    Sales is unit sales in thousands of units; $x_1$ is the price of the carseat; $x_2$ is an indicator variable, where $x_2=0$ if the store is in a rural area, and $x_2=1$ if the store is in an urban area; and $x_3$ is another indicator variable, where $x_3=0$ if the store is outside the United States, and $x_3=0$ if the store is within the United States.

(d) We can reject the null hypothesis $H_0: \beta_j=0$ for the Price and USYes predictors; their p-values are near 0.  We fail to reject the null hypothesis for UrbanYes; with a p-value of 0.936, we have no significant evidence that the difference in sales based on the urban status of the store was not due to random chance.

(e) We now fit a linear model based only on Price and USYes predictors.
    ```{r}
    lm.fit2 <- lm(Sales ~ Price + US, data = Carseats)
    summary(lm.fit2)
    ```
(f) With a virtually identical $R^2$ statistic of 0.2393 the models in part (a) and (e) both fit the data modestly well.  We can interpret this statistic to mean that the models explain roughly 24% of the variability in the carseat sales can be explained by the carseat's price and whether the store was located inside or outside the US.

(g) We now obtain 95% confidence intervals for the coefficients of the model in part (e):
    ```{r}
    confint(lm.fit2)
    ```

(h) To search for evidence of outlier observations in the model of part (e), we can plot the residuals of the model.
    ```{r}
    plot(residuals.lm(lm.fit2))
    ```

    We do not visually see evidence of outliers in our residual plot.  Next, we can search for evidence of high leverage observations by plotting leverage statistics for each observation.
    ```{r}
    plot(hatvalues(lm.fit2))
    ```

    Although we did not observe outliers, we do now see evidence of a few unusually leveraged observations.  We can identify the particular observation with the largest leverage statistic.
    ```{r}
    which.max(hatvalues(lm.fit2))
    ```
    
    Now we know that the observation with index 43 has the largest leverage statistic.
