...
First, we initialize the necessary libraries.
```{r}
library(ISLR2)
library(magrittr)
library(keras)
...
For a single layer network on Hitters data, we load the Hitters data and split it to training and test set.
```{r}
...
As a baseline, we fit a linear model and obtain the mean absolute prediction error, which is 254.6687.
```{r}
...
Last, we fit the neural network with library keras.  We set up a model structure describing the network.  The object "modnn" has a single hidden layer with 50 hidden units and a ReLU activation function.  It also has a dropout layer where a random 40% of the 50 activations from the previous layer are set to 0 during every iteration of SGD.  The output layer has no activation function, meaning the model provides a single quantitative output.
```{r}
...
Now we minimize squared error loss while tracking mean absolute error on the training data (and validation data if available).
```{r}
...
To construct a multi-layer network from the MNIST digit data, we first load and separate the data.
```{r}
mnist <- dataset_mnist()
...
Now we "one-hot" encode the class label.
```{r}
...
Because NNs can be sensitive to scale of inputs, we rescale each to a range from 0 to 1.
```{r}
x_train <- x_train/255
x_test <- x_test/255
```

Now we fit our neural network.
```{r}
...
Next we add model details specifying the fitting algorithm.
```{r}
modelnn %>% compile(loss='categorical_crossentropy', 
  optimizer=optimizer_rmsprop(), metrics=c("accuracy"))
```

Now we supply the training data and fit the model!
```{r}
system.time(
  history <- modelnn %>% 
...
We obtain the test error by comparing predicted and true class labels.
```{r}
...
We can demonstrate multiclass logistic regression by omitting the hidden layers.
```{r}
...
We fit a CNN to the CIFAR100 data, similar to the MNIST data.
```{r}
...
We standardize input input range and one-hot encode the response factors, producing a 100 column binary matrix.
```{r}
x_train <- x_train / 255
x_test <- x_test / 255
...
Let's peek at some of the training images.
```{r}
library(jpeg)
...
We specify a CNN of moderate size.
```{r}
...
Finally, we can specify the fitting algorithm and fit the model.
```{r}
...
Now we use a CNN which has been pretrained on the "Imagenet" database to classify natural images.
```{r}
img_dir <- "book_images"
...
Now we load the trained network with 50 layers.
```{r}
model <- application_resnet50(weights = "imagenet")
```

Now we classify the six images, returning the top three class choices by probability.
```{r}
...
We can perform document classification on the IMDb dataset.  First, limit dictionary size to the 10,000 most frequently-used words and tokens.  Also, we unpack the list of lists.
```{r}
max_features <- 10000
...
We create a function that provides a simple interface to the dictionary.
```{r}
...
Next, we "one-hot" encode each document in a list of documents, returning a binary matrix in sparse-matrix format, saving considerable memory.
```{r}
library(Matrix)
...
Words that appear more than once in any given document get recorded as a one.
```{r}
...
We create a validation set of 2,000.
```{r}
set.seed(3)
ival <- sample(seq(along = y_train), 2000)
```

We fit a lasso logistic regression model using glmnet().  We evaluate its performance on the validation data and plot accuracy as a function of the shrinkage parameter $\lambda$.
```{r}
...
We be sure to adjust the plotting window before making a plot.
```{r}
par(mar = c(4, 4, 4, 4), mfrow = c(1, 1))
plot(-log(fitlm$lambda), acclmv)
```

Now we fit a fully-connected NN with 2 hidden layers, each with 16 units and ReLU activation.
```{r}
model <- keras_model_sequential() %>%
...
```
To compute the test accuracy, we use the test data set as the `validation_data`.
```{r}
history <- model %>% fit(
...
history<-model %>% fit(x_train_1h[-ival, ], y_train[-ival],
  epochs = 20, batch_size = 512, validation_data = list(x_train_1h[ival, ], y_train[ival]))
...
We fit a simple Long- Short-Term Memory RNN for sentiment analysis with the IMDb movie-review data.  First, calculate the document lengths.
```{r}
...
Notice that 91% of documents have fewer than 500 words.  We restrict document lengths to the last L=500 words and pad the beginning of shorter ones with blanks. We peek at the last few words from the first document.
```{r}
maxlen <- 500
...
The first layer of our RNN one-hot encodes each document as a 500x10,000 matrix, then maps the 10,000 dimensions down to 32.
```{r}
model <- keras_model_sequential() %>%
...
The second layer is a LSTM with 32 units, and the output layer is a single sigmoid for binary classification task.
```{r}
model %>% compile(optimizer = "rmsprop",
...
Now we fit models for time series prediction.  First, set up the data and standardize variables.
```{r}
xdata <- data.matrix(
...
We write functions to create lagged versions of the three time series.
```{r}
lagm <- function(x, k = 1) {
   n <- nrow(x)
...
Now we create a data frame with lags as well as the response variable.
```{r}
...
Now we fit a linear autoregression model to the training data and predict on the test data, computing $R^2$.
```{r}
...
We refit the model including day of week.
```{r}
arframed <-
...
The fit our RNN, we have to reshape the data.  It expects a sequence of L=5 features vectors for each observation, our lagged versions going back L time points.
```{r}
n <- nrow(arframe)
...
Now we proceed with RNN using 12 hidden units.
```{r}
model <- keras_model_sequential() %>%
...
We fit our model , supplying fit function with test data as validation data.
```{r}
history <- model %>% fit(
...
We COULD replace the "keras_model_sequential()" command with the following, since "layer_flatten()" simply takes the input sequence and turns it into a long vector of predictors.
```{r}
model <- keras_model_sequential() %>%
...
We extract  the model matrix x which includes the day_of_week variable.
```{r}
x <- model.matrix(log_volume ~ . - 1, data = arframed)
colnames(x)
```

Finally, we fit a nonlinear AR model.
```{r}
...
log_model <- glm(y_train ~ studentNo + balance + income, family = binomial(link = logit), data = train)
...
```
In the table, we see that the logistic regression model and the neural net model performed almost identically. This is because we can think of a logistic regression model as a single layer neural net with a sigmoid activation function. The optimal parameters for both logistical regression and the single layer neural net with a sigmoid activation function are mathematically identical, so it makes sense that the models performed similarly. 
