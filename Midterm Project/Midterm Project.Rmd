---
title: "Math 533 Midterm Project"
...
We focus here on the support vector machines (SVM), one of the best "out of the box" classifiers that can be readily employed from a library.  First, we must understand the maximal margin classifier, as the SVM is a generalization of this simpler tool. The maximal margin classifier is elegant in practice, but it cannot be applied to most data sets since it requires that the classes be separable by a linear boundary.  Once we understand support vector machines, we will study support vector classifiers, a more broadly applicable extension of the maximal margin classifier which can be used more broadly.  Third, we discuss the support vector machine, a further extension of the support vector classifier which can accommodate non-linear class boundaries through the use of "kernels".  These classifiers are designed for use with binary classes, but we will then discuss how to use them in the case of three or more classes.  Finally, we probe the connections between SVMs and logistic regression which interestingly weren't clear until after SVMs had already been developed.
...
We can also make use of the "magnitude of $f(x^*)$.  "We say that if $f(x^*)$ is far from zero, then this means that $x^*$ is located near the hyperplan, and so we are less certain about the class assignment for $x^*$."  
...
To decide which hyperplane to use as a decision boundary (out of an infinite number of possibilities), we natural choice is the maximal margin hyperplane.  Notice the relation to the name "maximal margin classifier" here.  "This is the separating hyperplane that is farthest from the training observations.  To find it, we can compute the (perpendicular) distance from each training observation to a given separating hyperplane; the smallest such distance is the minimal distance from the observations to the hyperplane is known as the margin."  Although the maximal margin classifier is often successful, it can also lead to overfitting when the number of observations is large.

As before, if $\beta_0, \beta_1, \dots, \beta_p$ are the coefficients of the maximal margin hyperplane, then "the maximal margin classifier classifies the test observation $x^*$ based [simply] on the sign of $f(x^*) = \beta_0 + \beta_1 x^* + \dots + \beta_p x_p^*$.  We can think of the maximal margin hyperplane as representing the "mid-line of the widest 'slab' that we can insert between the classes."

We use the term "support vectors" to indicate the training observations that are equidistant from the maximal margin hyperplane.  This term reflects that the observations are vectors in p-dimensional space, and they "support" the maximal margin hyperplane in the sense that if these points were moved slightly then the entire maximal margin hyperplane would move as well.  "Interestingly, the maximal margin hyperplane depends directly on the support vectors, but not on the observations: a movement to any of the other observations would not affect the separating hyperplane" as long as that movement doesn't cause the observation to cross the margin boundary.  "The fact that the maximal margin hyperplane depends directly on only a small subset of observations is an important property" which will affect our discussion of support vector classifiers and support vector machines.
...
In many cases, observations belonging to two classes are separable by a hyperplane, so maximal margin classifiers cannot be used.  "In fact, even if a separating hyperplane does exist, then there are instances in which a classifier based on a separating hyperplane might not by desirable."  It is necessarily true that a classifier based on a separating hyperplane will perfectly classify all training observations, which unfortunately can lead to sensitivity to individual observations; sometimes a single observations can lead to a dramatic change in the maximal margin hyperplane.  This sensitivity to very few observations suggests overfitting may occur on the training data set.

In these cases we may consider a classifier based on a hyperplane that does not perfectly separate the two cases.  We may do this to increase robustness to individual observations and to attain better classification of most of the training observations.  The philosophy of the support vector classifier (or soft margin classifier) is that "it could be worthwhile to misclassify a few training observations in order to do a better job in classifying the remaining observations."
...
$$
$$
  (9.15): \epsilon_{i} \geq 0, \quad \sum_{i=1}^{n}\epsilon_{i}\leq C
$$
...
We can think of C as a "budget" for the amount that the margin can be violated by the observations.  If $C$ is 0, then there is no "budget" for violations and we have ourselves a maximal margin classifier.  In practice, "C is treated as a tuning parameter that is generally chosen via cross-validation."  Then, we use the $C$ in (9.15) to be the controller for the bias-variance trade-oﬀ of the statistical learning technique. According to the text, we know ``When $C$ is small, we seek narrow margins that are rarely violated; this amounts to a classifier that is highly fit to the data, which may have low bias but high variance. On the other hand, when $C$ is larger, the margin is wider and we allow more violations to it; this amounts to fitting the data less hard and obtaining a classifier that is potentially more biased but may have lower variance."

The optimization problem for $C$ has a "very interesting property: it turns out that only observations that either lie on the margin or that violate the margin will affect the hyperplane, and hence the classifier obtained."  Thus, observations that are strictly "correct" relative to the margin do not affect the support vector classifier.  We again use the term "support vectors" like before, but now for observations that lie directly on the margin or the wrong side of the margin for their class.

Since "the support vector classifier's decision rule is based only on a potentially small subset of the training observations (the support vectors), ... it is quite robust to the behavior of observations that are far away from the hyperplane."  This property is potentially useful and makes support vector classifiers distinct from other classification methods we have studied such as linear discriminant analysis.
...
But how does this lead to a non-linear decision boundary?  When we enlarge the feature space, the decision boundary resulting from the above maximization rule (9.16) is actually linear.  But, back in the original feature space, the decision boundary is of the form $q(x) = 0$ where $q$ is a quadratic polynomial whose solutions are generally non linear.  Additionally, we may choose to enlarge the feature space with higher-order polynomial terms, interaction terms, or other non-linear functions of the predictors.  As there are many possible ways to enlarge the feature space, we should be careful not to become overzealous, ending up with large, unwieldy number of features, rendering computation unmanageable.  This is where the support vector machine comes into play , which "allows us to enlarge the feature space used by the support vector classifier in a ways that leads to efficient computations."
...
One of the central advantages of using a kernel rather than simply enlarging the feature space using functions of the original features is computational.  Using kernels, we need only to compute $K(x_i , x'_i)$ for all $n \choose 2$ distinct pairs $i$, $i'$.  We can do this "without explicitly working in the enlarged feature space.  This is important because in many applications of SVMs, the enlarged feature space is so large that computations are intractable."
...
An interesting characteristic of the support vector classifier is that "only support vectors play a role in the classifier obtained; observations on the correct side of the margin do not affect it."  Now we understand why: the loss function defined above is exactly zero for observations for which $y_{i}(\beta_{0}+\beta_{1}x_{i1}+\beta_{2}x_{i2}+\dots+\beta_{p}x_{ip}) \geq 1$, corresponding the observations on the correct side of the margin.  This is in notable contrast to the logistic regression loss function where is not exactly zero anywhere (though it does get very small for observations far away from the decision boundary).

We have seen that the use of kernels to enlarge features spaces is a powerful aspect of SVMs.  But are SVMs unique in their ability to harness kernels like this?  No.  It turns out we can also perform logistic regression or many other classification methods using non-linear kernels.  However, we typically encounter the use of non-linear kernels in SVMs due to the history of when these techniques were popularized.  

One further application of SVMs is in performing regression for quantitative responses, called support vector regression.  Support vector regression seeks coefficients that maximize a different type of loss than sum of squared residuals, where only residuals large in absolute value than some positive constant contribute to the loss function.  This extends the idea of "margins" in support vector classifiers to the regression setting.
...
$$
\mathcal{L}_m(F_m) = \sum_{i=1}^N\ell(y_i, f_{m-1}(\boldsymbol x_i) + F_m(\boldsymbol x_i)) + \Omega(F_m) + \text{const}.
$$
The second order Taylor expansion is as follows:
$$
\mathcal{L}_m(F_m)\approx \sum_{i=1}^N\left[\ell(y_i, f_{m-1}(\boldsymbol x_i)) + g_{im}F_m(\boldsymbol x_i) + \frac{1}{2}h_{im}F_m^2(\boldsymbol x_i)\right] + \Omega(F_m) + \text{const},
$$
where $h_{im}$ is the Hessian
$$
h_{im} = \left[\frac{\partial^2\ell(y_i, f(\boldsymbol x_i))}{\partial f(\boldsymbol x_i)^2}\right]_{f = f_{m-1}}.
$$
With regression trees, $F(\boldsymbol x) = w_{q(\boldsymbol x)}$, where $q:\mathbb{R}^D \rightarrow \{1,\dots,J\}$ controls which leaf node $\boldsymbol x$ belongs to, and $\boldsymbol w\in\mathbb{R}^J$ are the leaf weights. Thus, after some transformation, the 2nd order Taylor expansion can be written as
$$
\mathcal{L}_m(q,\boldsymbol w) \approx \sum_{j=1}^J\left[G_{jm}w_j + \frac{1}{2}(H_{jm}+\lambda)w_j^2\right] + \gamma J,
$$
where $G_{jm} = \sum_{i\in I_j}g_{im}$, $H_{jm} = \sum_{i\in I_j} h_{im}$, and $I_j = \{i: q(\boldsymbol x_i) = j\}$ is the set of indices of data points assigned to the $j$'th leaf.

Then, the optimal weights are
$$
w_j^* = - \frac{G_{jm}}{H_{jm}+\lambda},
$$
so the loss at different tree structures $q$ is then
$$
\mathcal{L}(q, \boldsymbol w^*) = -\frac{1}{2}\sum_{j=1}^J\frac{G_{jm}^2}{H_{jm}+\lambda} + \gamma J,
$$
which can be greedily optimized using a recursive node splitting procedure.
