...
After conducted some transformation, the final objective becomes 
...
Slack variables introduced if the data is not lineraly separable. We use the soft margin constraints to replace the ard constraints, then we have the new objectve as following:
$$
...
$$
The bias term as following after kernelizing
...
The goal is to convert the output of an SVM to a probability. We have following probability
$$
...
For the data points lie on the correct side of the decision boundary, we have $ \xi_n = 0 $; for the others, we have $ \xi_n = 1 - \tilde{y}_nf(\boldsymbol x_n) $. Then, we can rewrite the bojective euqation of Soft margin classifiers as following:
$$
...
$$
Compared to the logistic regression oprimizes:
$$
  \mathcal{L}(w) = \sum_{i=1}^{N} \ell_{ll} (\tilde{y_n},f(x_n)) + \lambda ||w||^2
$$
Where the log loss function as below:
$$
  \ell_{ll} (\tilde{y_n},\eta) = log(1+e^{-\tilde{y_n}\eta})
$$
There are two major difference between the log loss and hinge loss:
\
1: The hinge loss is piecewise linear, so we cannot use regular gradient methods to optimize it. 
\
...
The kernel ridge regression as follows:
$$
...
For regression models, averaging is a good way to combine predictions, and for classifiers is is sometimes better to take a majority vote of the outputs. 
...
Exponential loss pus a lot of weight on misclassified examples, which makes the method sensitive to outliers. Since $e^{-\tilde{y}f}$ is not the logarithm of a pmf for binary variables $\tilde{y}\in\{-1,+1\}$, we can't recover probability estimates from $f(\boldsymbol x)$. 
